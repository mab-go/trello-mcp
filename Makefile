# trello-mcp -- Run 'make' or 'make help' to see available commands

.DEFAULT_GOAL := help

BIN         := ./bin
BINARY      := $(BIN)/trello-mcp
MODULE      := github.com/mab-go/trello-mcp
...
build: ## Build binary to ./bin/trello-mcp with version ldflags
	@mkdir -p $(BIN)
	go build -o $(BINARY) -ldflags "$(LDFLAGS)" ./cmd/trello-mcp

install: ## Run go install with same ldflags (installs to GOPATH/bin or GOBIN)
	go install -ldflags "$(LDFLAGS)" ./cmd/trello-mcp
...
run: ## Run via go run (optional: ARGS="--flags")
	go run -ldflags "$(LDFLAGS)" ./cmd/trello-mcp $(ARGS)
